...
void main()
{
    ...
    while(1)
    {
        ...
        if(choice==4)
            printf("Enter the arrival and burst time & priority of processes: ");
        else
        ...
        if(choice==1){
            ...
            for(int i=0;i<n;i++){
                if(i==0)
                    ct[index[i]]=time=at[index[i]]+bt[index[i]];
                else{
                    if(at[index[i]]<=time)
                        ct[index[i]]=time=time+bt[index[i]];
                    else
                        ct[index[i]]=time=at[index[i]]+bt[index[i]];
                }
            }
